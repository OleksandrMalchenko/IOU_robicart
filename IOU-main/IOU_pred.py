...
def read_content_pred(xml_file: str):
    ...
    if object is not None:
        for boxes in root.iter('object'):    
            ...
            ymin = int(round(float(boxes.find("boundingbox/y1").text)))
            xmin = int(round(float(boxes.find("boundingbox/x1").text)))
            #xmin = int(boxes.find("boundingbox/x1").text)
            #ymax = int(boxes.find("boundingbox/y2").text)
            ymax = int(round(float(boxes.find("boundingbox/y2").text)))
            #xmax = int(boxes.find("boundingbox/x2").text)
            xmax = int(round(float(boxes.find("boundingbox/x2").text)))
...
def main(folder_name:str):
    ...
    rootDirectory = 'Images/Test1_office'
    ...
    for filename in os.listdir(predictionDirectory):
        #currentAnnotation = os.path.join(annotationDirectory, filename)
        #namePred = os.path.splitext(filename)[0] + '_pred.xml'
        namePred = os.path.splitext(filename)[0] + '.xml'
        namePred.strip()
        print(namePred)
        if namePred.startswith('.'):
         print(f"Found a filename starting with a dot: {namePred}")
        else:
            #bbox_array_anot = []
            bbox_array_pred = []
